...
export default async function handler(req, res) {
    ...
    if (req.method === 'GET') {
        try {
            ...
            // Find the last inserted item from the event_news collection
            const lastInsertedEventNewsItem = await db.collection('event_news').findOne({}, { sort: { _id: -1 }, projection: { _id: 0 } });

            res.status(200).json(lastInsertedEventNewsItem);
        } catch (error) {
            console.error(error);
